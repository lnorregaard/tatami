...
public class ModeratorTimelineServiceTest extends AbstractCassandraTatamiTest {
    ...
    public void blockStatus() {
        ...
        List<UUID> list = statusStateGroupRepository.findStatuses("BLOCKED",group.getGroupId().toString(),null,null,10, null);
    ...
    public void approveStatus() {
        ...
        List<UUID> list = statusStateGroupRepository.findStatuses("APPROVED",group.getGroupId().toString(),null,null,10, null);
        assertThat(list, notNullValue());
        assertThat(list.size(), is(1));
        assertThat(list.iterator().next(),is(status.getStatusId()));
